...
@section('contenido')
    <main class="flex bg-gray-100">
        ...
        <div class="w-full flex flex-col h-screen overflow-y-hidden">
            <p class="flex justify-center text-4xl text-center font-bold">Pedidos</p>
            <nav class=" h-16 flex justify-end py-4 px-16">
                <a href="{{route('otherU.pedidos')}}" class="border border-green-700 rounded px-4 pt-1 h-10 bg-white text-green-800 mx-2">Actualizar</a>
            </nav>
                ...
                <table class="table-fixed w-full">
                    <thead>
                        <tr class="bg-green-700 text-white">
                            ...
                            <th class="w-2 py-3">prestado a </th>
                            <th class="w-2 py-3">material</th>
                    ...
                    <tbody>
                        @foreach($pres as $rod)
                            <tr>
                                ...
                                <td class="py-4 text-center">{{$rod->preu->name}}</td>
                                <td class="py-4 text-center">{{$rod->pretm->descripcion}}</td>
                                <td class="py-4 text-center">{{$rod->pree->descripcion}}</td>
                                <td class="p-4 flex justify-center">
                                    <form action="{{ route('pedido.destroy',$rod->id)}}" method="POST">
                                        @csrf
                                        @method('delete')    
                                    ...
                                    <a href="{{ route('pedido.edit',$rod->id)}}" class="bg-green-500 text-white px-3 py-1 rounded-sm">
