@extends('layouts.app')
@section('title', 'Material De Practica')
@section('contenido') 
    <main class="flex bg-gray-100">
        ...
        <p class="flex justify-center text-4xl text-center font-bold">Material de practica</p>
            <nav class=" h-16 flex justify-end py-4 px-16">
                <a href="{{route('TipoMaterialPractica.crudTMP')}}" class="border border-green-700 rounded px-4 pt-1 h-10 bg-white text-green-700 mx-2">Catalogo Material de practica</a>
                <a href="{{route('MaterialPractica.crudMP')}}" class="border border-green-700 rounded px-4 pt-1 h-10 bg-white text-green-700 mx-2">Actualizar</a>
                <a href="{{route('MaterialPractica.registerMP')}}" class="text-white rounded px-4 pt-1 h-10 bg-green-700 mx-2 hover:bg-green-800">Nuevo</a>
            </nav>
                ...
                <table class="table-fixed w-full">
                    <thead>
                        <tr class="bg-green-700 text-white">
                            ...
                            <th class="w-28 py-4 ... ">Material</th>
                            <th class="w-28 py-4 ... ">cantidad</th>
                            <th class="w-28 py-4 ... ">descripcion</th>
                            <th class="w-28 py-4 ... ">Actions</th>
                        </tr>
                    </thead>
                    <tbody>
                        @foreach($material as $rod)
                            <tr>
                                <td class="py-4 text-center">{{$rod->id}}</td>
                                <td class="p-4 text-center">{{$rod->tipom->descripcion}}</td>
                                <td class="p-4 text-center">{{$rod->cantidad}}</td>
                                <td class="p-4 text-center">{{$rod->descripcion}}</td>
                                <td class="p-4 flex justify-center">
                                    <form action="{{ route('material.destroy',$rod->id)}}" method="POST">
                                        @csrf
                                        @method('delete')    
                                    ...
                                    <a href="{{ route('material.edit',$rod->id)}}" class="bg-green-500 text-white px-3 py-1 rounded-sm">
